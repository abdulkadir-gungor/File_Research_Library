...
from  lib.image.jpg.JPGBytes import JPGBytes as jpg
import os
#
#
#
##### Main Program #####
def jpgInspector(filename:str, jpgBytes:bytes):
    while True:
        while True:
            try:
                jpg_info = jpg(jpgBytes=jpgBytes , filename=filename)
                break
            except:
                return
        while True:
            selection =  menu_1(filename=filename)
            if selection != -1:
...
# screen_1
def screen_1(filename:str):
    #
    txt0_tmp  = '\t  Filename : "{}"'.format(filename)
    txt2_tmp =  '\t     *****   MENU   *****'
    if len(txt0_tmp) > len(txt2_tmp):
        len_str = len(txt0_tmp) + 2
    else:
        len_str = len(txt2_tmp) + 2
    #
    txt1_tmp  = '\t'+('-'*len_str)
...
# Ana Menü
def menu_1(filename:str) -> int:
    while True:
        screenClear()
        screen_1(filename=filename)
        print()
        print()
